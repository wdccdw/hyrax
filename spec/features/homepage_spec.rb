RSpec.describe "The homepage", :clean_repo do
  ...
  let(:work1) { create(:work, :public, title: ["Work 1"], date_uploaded: (DateTime.now - 1.day)) }
  let(:work2) { create(:work, :public, title: ["Work 2"], date_uploaded: (DateTime.now - 1.year)) }
  ...
  it 'shows recently uploaded' do
    ...
    within '#recently_uploaded' do
      ...
      expect(page).to_not have_link work2.title.first
      ...
      expect(DateTime.parse(work2.to_solr['system_create_dtsi'])).to be > DateTime.parse(work1.to_solr['system_create_dtsi'])
    end
  end
